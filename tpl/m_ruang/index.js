app.controller('RuangCtrl', function ($scope,  Data, $rootScope) {
    //init data
    var tableStateRef;
var Control_link = "m_ruang";
   ...
   $scope.callServer = function callServer(tableState) {
    ...
    Data.get("m_ruang/sekolah").then(function(data) {
    ...
    $scope.hitungTotal = function(){
        ...
        angular.forEach($scope.detJawab, function(value, key){
            value.luas = value.panjang*value.lebar;
            $scope.form.total += value.luas;
        });
    };
    $scope.update = function (form) {
        ...
        $scope.form = form;
        $scope.detJawab = [];
       Data.get('m_ruang/listdetail/' + form.id).then(function (data) {
    ...
    $scope.trash = function (row) {
        Swal.fire({
            ...
            if (result.value) {
                row.is_deleted = 1;
                Data.post('m_ruang/update/' + row.id, row).then(function (result) {
